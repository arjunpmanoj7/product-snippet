...
class PosQuotation(models.Model):
    ...
    order_ref = fields.Char(store=True, String="Order_ref")
    ...
    deliver_date = fields.Datetime(string='Deliver Date', default=fields.Datetime.now, readonly=True)
    ...
    def create_from_ui(self, orders):
        ...
        for line in self._order_fields(orders).get('lines'):
            test = line[2]
            if test:
                test.pop('price_manually_set', None)
    ...
    def search_read_new(self, quotation_id, field, line_fields):
        quotation_id = self.browse(quotation_id)
        quotation = quotation_id.read(field)
        if quotation:
            line_ids = self.env['book.order.line'].browse(quotation_id.lines.ids)
            lines = line_ids.read(fields=line_fields)
            quotation[0].update({
                'lines': lines
            })
            return quotation[0]
    ...
    @api.model
    def confirm_action(self, orders):
        print('anoop')
        print(self, orders)
        book_order = self.env['book.order'].search([('order_ref', '=', orders)])
        book_order.write({
            'state': 'confirmed',
        })
...
class PosQuotationLine(models.Model):
    ...
    tax_ids = fields.Many2many('account.tax', 'book_order_taxes_rel',
                               'order_id', 'tax_id', string='Taxes',
                               readonly=True)
    tax_ids_after_fiscal_position = fields.Many2many(
        'account.tax', 'bookk_order_after_taxes_rel', 'order_id', 'tax_id',
        string='Taxes After Fiscal Position')
    pack_lot_ids = fields.One2many('pos.pack.operation.lot', 'pos_order_line_id',
                                   string='Lot/serial Number')
    note = fields.Char(string='Orderline note')
    customer_note = fields.Char(string='note')
    mp_dirty = fields.Boolean(string="MP Dirty Parameter")
    mp_skip = fields.Boolean(string="MP Skip Parameter")
...
class PosSessions(models.Model):
    ...
    def _loader_params_book_order(self):
        return {
            'search_params': {
                'domain': [('state', '=', 'draft')],
                'fields': [
                    'name', 'partner_id', 'date_order', 'amount_total', 'book_order', 'phone', 'create_date',
                    'pickup_date',
                    'deliver_date', 'delivery_address', 'note', 'pricelist_id', 'lines', 'state'
                ],
            },
        }
    ...
    def _loader_params_book_order_line(self):
        return {
            'search_params': {
                'fields': [
                    'product_id', 'qty', 'note', 'mp_skip', 'mp_dirty'
                ],
            },
        }

    def _get_pos_ui_book_order(self, params):
        return self.env['book.order'].search_read(**params['search_params'])

    def _get_pos_ui_book_order_line(self, params):
        return self.env['book.order.line'].search_read(**params['search_params'])
